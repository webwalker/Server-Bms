...
public class WebUtil {
	...
	public static Map<String, Object> getRequestParamMap(
		...
		try {
			...
			if (method.equalsIgnoreCase("put")
				...
				String queryString = CodecUtil.decodeURL(StreamUtil
	...
	public static void downloadFile(HttpServletResponse response,
		...
		try {
			...
			String downloadedFileName = new String(
					originalFileName.getBytes("GBK"), "ISO8859_1"); // 防止中文乱码
	...
	public static String createCaptcha(HttpServletResponse response) {
		...
		try {
			...
			for (int i = 0; i < codeCount; i++) {
				// 获取随机验证码
				String validateCode = String.valueOf(codeSequence[random
						.nextInt(randomSeed)]);
				// 随机构造颜色值
