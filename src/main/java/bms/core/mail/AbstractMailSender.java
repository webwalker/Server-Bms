...
public abstract class AbstractMailSender implements MailSender {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(AbstractMailSender.class);
	...
	public final void send() {
		try {
			...
			if (MailConstant.Sender.IS_AUTH) {
				email.setAuthentication(MailConstant.Sender.AUTH_USERNAME,
						MailConstant.Sender.AUTH_PASSWORD);
			}
			// 判断是否开启 Debug 模式
			...
			// 设置 From 地址
			if (MailConstant.Sender.FROM != null
					&& !MailConstant.Sender.FROM.equals("")) {
	...
	public abstract void setContent(Email email, String content)
